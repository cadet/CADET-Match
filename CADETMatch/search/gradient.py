...
def run(cache, tools, creator):
    ...
    with path.open("a", newline="") as csvfile:
        ...
        gradCheck, newChildren = cache.toolbox.grad_search(
            gradCheck, pop, cache, writer, csvfile, hof, meta_hof, -1, check_all=True, filterOverlap=False
        )

        stalled, stallWarn, progressWarn = util.eval_population(
            cache.toolbox, cache, newChildren, writer, csvfile, hof, meta_hof, None, -1, result_data
        )
