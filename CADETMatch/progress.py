...
def write_results(cache, result_h5, result_data, gen_data, now, sim_start,
     ...
     with h5py.File(result_h5, 'w') as hf:
        ...
        hf.create_dataset("output_meta", data=meta_score_trans(cache, result_data['output_meta']), maxshape=(None, len(result_data['output_meta'][0])))
...
def update_results(cache, result_h5, result_data, gen_data, now, sim_start,
                  population_input, population_output,
                  data, hof_param, hof_param_transform,
                  data_meta, meta_param, meta_param_transform,
                  data_grad, grad_param, grad_param_transform):
...
def writeProgress(cache, generation, population, halloffame, meta_halloffame, grad_halloffame, progress_halloffame,
    ...
    if result_data is not None:
        ...
        if not result_h5.exists():
            write_results(cache, result_h5, result_data, gen_data, now, sim_start,
                  population_input, population_output,
                  data, hof_param, hof_param_transform,
                  data_meta, meta_param, meta_param_transform,
                  data_grad, grad_param, grad_param_transform)
