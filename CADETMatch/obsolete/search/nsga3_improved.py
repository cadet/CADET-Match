...
def setupDEAP(cache, fitness, grad_fitness, grad_search, map_function, creator, base, tools):
    ...
    if cache.adaptive:
        cache.toolbox.register(
            "mutate", util.mutationNSGA3_mutate, low=cache.MIN_VALUE, up=cache.MAX_VALUE, indpb=1.0 / len(cache.MIN_VALUE)
        )
        cache.toolbox.register(
            "force_mutate", util.mutationNSGA3_mutate, low=cache.MIN_VALUE, up=cache.MAX_VALUE, indpb=1.0 / len(cache.MIN_VALUE)
        )
    else:
        cache.toolbox.register(
            "mutate", util.mutPolynomialBounded, eta=1.0, low=cache.MIN_VALUE, up=cache.MAX_VALUE, indpb=1.0 / len(cache.MIN_VALUE)
        )
        cache.toolbox.register(
            "force_mutate", util.mutPolynomialBounded, eta=1.0, low=cache.MIN_VALUE, up=cache.MAX_VALUE, indpb=1.0 / len(cache.MIN_VALUE)
        )
