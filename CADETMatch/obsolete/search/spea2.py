...
def setupDEAP(cache, fitness, grad_fitness, grad_search, map_function, creator, base, tools):
    ...
    # else:
    cache.toolbox.register(
        "mutate", util.mutPolynomialBounded, eta=70.0, low=cache.MIN_VALUE, up=cache.MAX_VALUE, indpb=1.0 / len(cache.MIN_VALUE)
    )
    cache.toolbox.register(
        "force_mutate", util.mutPolynomialBounded, eta=70.0, low=cache.MIN_VALUE, up=cache.MAX_VALUE, indpb=1.0 / len(cache.MIN_VALUE)
    )
